...
return amountstatus;
	...
	public List<TransportVo> getRouteWiseStudentDetail(String location, String accyear, String routeNo) {
		...
		try {
			connection = JDBCConnection.getSeparateConnection();
			pstmt = connection.prepareStatement("SELECT CONCAT(cs.student_fname_var,' ',cs.student_lname_var) AS student,cs.student_id_int,cs.student_admissionno_var,ccd.`classdetails_name_var`,ccs.`classsection_name_var`,cl.`Location_Name`,cst.route_id,GROUP_CONCAT(DISTINCT cst.stage_id) stage_id  FROM `campus_student_route_stage_mapping` cst JOIN campus_student cs ON cst.mapped_id=cs.student_id_int JOIN campus_student_classdetails csc ON cs.student_id_int=csc.student_id_int AND cst.accyear=csc.fms_acadamicyear_id_int JOIN `campus_classdetail` ccd ON csc.classdetail_id_int=ccd.classdetail_id_int AND csc.locationId=ccd.locationId JOIN `campus_location` cl ON cl.`Location_Id`=cs.`locationId` JOIN `campus_classsection` ccs ON csc.classsection_id_int=ccs.classsection_id_int WHERE cl.`Location_Id`=? AND cst.accyear=? AND cst.route_id LIKE ?  GROUP BY mapped_id,route_id");
			pstmt.setString(1, location);
			pstmt.setString(2, accyear);
			pstmt.setString(3, routeNo);
	...
	public ArrayList<TransportVo> getFeeCollectionPaymodeReport(
		...
		try{
			   ...
			   // String modeofpayment;
			    ...
			    {
					conn = JDBCConnection.getSeparateConnection();
					pstmt=conn.prepareStatement("SELECT cft.termname,fc.paidDate,fc.reciept_no,CONCAT(cd.classdetails_name_var, ' ',cs.classsection_name_var) AS class, CONCAT(st.student_fname_var,' ',st.student_lname_var)AS student,fc.modeofpayment, SUM(fc.totalamount) AS totalamount FROM  campus_tranport_fee_collection_details fc JOIN campus_student_classdetails csc ON fc.admissionNo = csc.student_id_int AND fc.accYear=csc.fms_acadamicyear_id_int JOIN campus_student st  ON fc.admissionNo = st.student_id_int JOIN campus_parentchildrelation par ON par.stu_addmissionNo = fc.admissionNo JOIN  campus_parents pr ON pr.parentid = par.ParentID JOIN campus_classdetail cd ON cd.classdetail_id_int=csc.classdetail_id_int AND cd.locationId = csc.locationId JOIN campus_classsection cs ON cs.classsection_id_int=csc.classsection_id_int AND cs.locationId = csc.locationId  JOIN campus_fee_transport_termdetails cft ON cft.termid=fc.termcode where csc.locationId = ? and fc.accYear = ? and cs.classsection_id_int like ? and fc.termcode like ? and cd.classdetail_id_int like '%%' and fc.modeofpayment = 'CASH' group by fc.admissionNo order by fc.reciept_no and cd.classdetails_name_var and cs.classsection_name_var and st.student_fname_var and st.student_lname_var");
				...
				{
					conn = JDBCConnection.getSeparateConnection();
					pstmt=conn.prepareStatement("SELECT cft.termname,fc.paidDate,fc.reciept_no,CONCAT(cd.classdetails_name_var, ' ',cs.classsection_name_var) AS class, CONCAT(st.student_fname_var,' ',st.student_lname_var)AS student,fc.modeofpayment, SUM(fc.totalamount) AS totalamount FROM  campus_tranport_fee_collection_details fc JOIN campus_student_classdetails csc ON fc.admissionNo = csc.student_id_int AND fc.accYear=csc.fms_acadamicyear_id_int JOIN campus_student st  ON fc.admissionNo = st.student_id_int JOIN campus_parentchildrelation par ON par.stu_addmissionNo = fc.admissionNo JOIN  campus_parents pr ON pr.parentid = par.ParentID JOIN campus_classdetail cd ON cd.classdetail_id_int=csc.classdetail_id_int AND cd.locationId = csc.locationId JOIN campus_classsection cs ON cs.classsection_id_int=csc.classsection_id_int AND cs.locationId = csc.locationId  JOIN campus_fee_transport_termdetails cft ON cft.termid=fc.termcode where csc.locationId = ? and fc.accYear = ? and cs.classsection_id_int like ? and fc.termcode like ? and cd.classdetail_id_int like '%%' and fc.modeofpayment = 'DD' group by fc.admissionNo order by fc.reciept_no and cd.classdetails_name_var and cs.classsection_name_var and st.student_fname_var and st.student_lname_var");
					//System.out.println(pstmt);
				...
				{
					conn = JDBCConnection.getSeparateConnection();
					pstmt=conn.prepareStatement("SELECT cft.termname,fc.paidDate,fc.reciept_no,CONCAT(cd.classdetails_name_var, ' ',cs.classsection_name_var) AS class, CONCAT(st.student_fname_var,' ',st.student_lname_var)AS student,fc.modeofpayment, SUM(fc.totalamount) AS totalamount FROM  campus_tranport_fee_collection_details fc JOIN campus_student_classdetails csc ON fc.admissionNo = csc.student_id_int AND fc.accYear=csc.fms_acadamicyear_id_int JOIN campus_student st  ON fc.admissionNo = st.student_id_int JOIN campus_parentchildrelation par ON par.stu_addmissionNo = fc.admissionNo JOIN  campus_parents pr ON pr.parentid = par.ParentID JOIN campus_classdetail cd ON cd.classdetail_id_int=csc.classdetail_id_int AND cd.locationId = csc.locationId JOIN campus_classsection cs ON cs.classsection_id_int=csc.classsection_id_int AND cs.locationId = csc.locationId  JOIN campus_fee_transport_termdetails cft ON cft.termid=fc.termcode where csc.locationId = ? and fc.accYear = ? and cs.classsection_id_int like ? and fc.termcode like ? and cd.classdetail_id_int like '%%' and fc.modeofpayment = 'CHEQUE' group by fc.admissionNo order by fc.reciept_no and cd.classdetails_name_var and cs.classsection_name_var and st.student_fname_var and st.student_lname_var");
	...
	public ArrayList<TransportVo> getfeecollectiondatelist(String locationid, String accyear, String classid,
		...
		try{
			...
			{
				conn = JDBCConnection.getSeparateConnection();
				pstmt=conn.prepareStatement("SELECT cft.termname,fc.paidDate,fc.reciept_no,CONCAT(cd.classdetails_name_var, ' ',cs.classsection_name_var) AS class, CONCAT(st.student_fname_var,' ',st.student_lname_var)AS student,fc.modeofpayment, SUM(fc.totalamount) AS totalamount FROM  campus_tranport_fee_collection_details fc JOIN campus_student_classdetails csc ON fc.admissionNo = csc.student_id_int AND fc.accYear=csc.fms_acadamicyear_id_int JOIN campus_student st  ON fc.admissionNo = st.student_id_int JOIN campus_parentchildrelation par ON par.stu_addmissionNo = fc.admissionNo JOIN  campus_parents pr ON pr.parentid = par.ParentID JOIN campus_classdetail cd ON cd.classdetail_id_int=csc.classdetail_id_int AND cd.locationId = csc.locationId JOIN campus_classsection cs ON cs.classsection_id_int=csc.classsection_id_int AND cs.locationId = csc.locationId  JOIN campus_fee_transport_termdetails cft ON cft.termid=fc.termcode where csc.locationId = ? and fc.accYear = ? and cs.classsection_id_int like '%%' and fc.termcode like ? and cd.classdetail_id_int like '%%' and fc.modeofpayment = 'ONLINE' AND paidDate >= ? AND paidDate <= ? GROUP BY fc.admissionNo order by fc.reciept_no , cd.classdetails_name_var, cs.classsection_name_var , st.student_fname_var , st.student_lname_var");
			}
			else if(paymode.equalsIgnoreCase("CASH"))
		    {
				conn = JDBCConnection.getSeparateConnection();
				pstmt=conn.prepareStatement("SELECT cft.termname,fc.paidDate,fc.reciept_no,CONCAT(cd.classdetails_name_var, ' ',cs.classsection_name_var) AS class, CONCAT(st.student_fname_var,' ',st.student_lname_var)AS student,fc.modeofpayment, SUM(fc.totalamount) AS totalamount FROM  campus_tranport_fee_collection_details fc JOIN campus_student_classdetails csc ON fc.admissionNo = csc.student_id_int AND fc.accYear=csc.fms_acadamicyear_id_int JOIN campus_student st  ON fc.admissionNo = st.student_id_int JOIN campus_parentchildrelation par ON par.stu_addmissionNo = fc.admissionNo JOIN  campus_parents pr ON pr.parentid = par.ParentID JOIN campus_classdetail cd ON cd.classdetail_id_int=csc.classdetail_id_int AND cd.locationId = csc.locationId JOIN campus_classsection cs ON cs.classsection_id_int=csc.classsection_id_int AND cs.locationId = csc.locationId  JOIN campus_fee_transport_termdetails cft ON cft.termid=fc.termcode where csc.locationId = ? and fc.accYear = ? and cs.classsection_id_int like '%%' and fc.termcode like ? and cd.classdetail_id_int like '%%' and fc.modeofpayment = 'CASH' AND paidDate >= ? AND paidDate <= ? GROUP BY fc.admissionNo order by fc.reciept_no , cd.classdetails_name_var , cs.classsection_name_var , st.student_fname_var , st.student_lname_var");
			...
			{
				conn = JDBCConnection.getSeparateConnection();
				pstmt=conn.prepareStatement("SELECT cft.termname,fc.paidDate,fc.reciept_no,CONCAT(cd.classdetails_name_var, ' ',cs.classsection_name_var) AS class, CONCAT(st.student_fname_var,' ',st.student_lname_var)AS student,fc.modeofpayment, SUM(fc.totalamount) AS totalamount FROM  campus_tranport_fee_collection_details fc JOIN campus_student_classdetails csc ON fc.admissionNo = csc.student_id_int AND fc.accYear=csc.fms_acadamicyear_id_int JOIN campus_student st  ON fc.admissionNo = st.student_id_int JOIN campus_parentchildrelation par ON par.stu_addmissionNo = fc.admissionNo JOIN  campus_parents pr ON pr.parentid = par.ParentID JOIN campus_classdetail cd ON cd.classdetail_id_int=csc.classdetail_id_int AND cd.locationId = csc.locationId JOIN campus_classsection cs ON cs.classsection_id_int=csc.classsection_id_int AND cs.locationId = csc.locationId  JOIN campus_fee_transport_termdetails cft ON cft.termid=fc.termcode where csc.locationId = ? and fc.accYear = ? and cs.classsection_id_int like '%%' and fc.termcode like ? and cd.classdetail_id_int like '%%' and fc.modeofpayment = 'DD' AND paidDate >= ? AND paidDate <= ? GROUP BY fc.admissionNo order by fc.reciept_no , cd.classdetails_name_var , cs.classsection_name_var , st.student_fname_var ,st.student_lname_var");
				//System.out.println(pstmt);
			...
			{
				conn = JDBCConnection.getSeparateConnection();
				pstmt=conn.prepareStatement("SELECT cft.termname,fc.paidDate,fc.reciept_no,CONCAT(cd.classdetails_name_var, ' ',cs.classsection_name_var) AS class, CONCAT(st.student_fname_var,' ',st.student_lname_var)AS student,fc.modeofpayment, SUM(fc.totalamount) AS totalamount FROM  campus_tranport_fee_collection_details fc JOIN campus_student_classdetails csc ON fc.admissionNo = csc.student_id_int AND fc.accYear=csc.fms_acadamicyear_id_int JOIN campus_student st  ON fc.admissionNo = st.student_id_int JOIN campus_parentchildrelation par ON par.stu_addmissionNo = fc.admissionNo JOIN  campus_parents pr ON pr.parentid = par.ParentID JOIN campus_classdetail cd ON cd.classdetail_id_int=csc.classdetail_id_int AND cd.locationId = csc.locationId JOIN campus_classsection cs ON cs.classsection_id_int=csc.classsection_id_int AND cs.locationId = csc.locationId  JOIN campus_fee_transport_termdetails cft ON cft.termid=fc.termcode where csc.locationId = ? and fc.accYear = ? and cs.classsection_id_int like '%%' and fc.termcode like ? and cd.classdetail_id_int like '%%' and fc.modeofpayment = 'CHEQUE' AND paidDate >= ? AND paidDate <= ? GROUP BY fc.admissionNo order by fc.reciept_no , cd.classdetails_name_var , cs.classsection_name_var , st.student_fname_var , st.student_lname_var");
	...
	try {
		...
		if(!(stop=="all"))
		{
			...
			while(resultSet1.next()) {
				 ...
				 obj.setClassname(resultSet1.getString("classdetails_name_var"));
				 obj.setSection_name(resultSet1.getString("classsection_name_var"));
				 obj.setDescription(resultSet1.getString("description"));
				 obj.setStage_name(resultSet1.getString("stage_name"));
		...
		resultSet = pstmt.executeQuery();
		 while(resultSet.next()) {
			 ...
			 obj.setSection_name(resultSet.getString("classsection_name_var"));
			 obj.setDescription(resultSet.getString("description"));
			 obj.setStage_name(resultSet.getString("stage_name"));
			 obj.setSmsnumber(resultSet.getString("smsNo"));
			 list.add(obj);
		 }
		}
