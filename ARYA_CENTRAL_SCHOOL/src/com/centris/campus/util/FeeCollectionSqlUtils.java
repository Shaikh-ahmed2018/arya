...
*/	
	...
	public static final String GET_FEECOLLECTION_AMOUNT ="select fm.FeeCode,fm.FeeName,fm.IsConcession,fsd.feeAmount from campus_fee_master fm,campus_fee_setup fs,campus_fee_setupdetails fsd where fs.feeSettingcode=fsd.feeSettingCode and fm.FeeCode=fsd.feecode and fs.ClassCode=? and fs.AccyearCode=? and fs.Termcode=? and fs.specialization=? AND fsd.feecode NOT IN (SELECT feeCode FROM `campus_fee_specialfee_setup` WHERE `student_id`=? AND `accyear`=? AND termid=?)";
	public static final String GET_TRANSPORT_FEE = "SELECT fsd.feeAmount,fsd.stagecode from campus_fee_stagesetupdetails fsd ,campus_fee_stagesetup fs where  fsd.stageSettingCode=fs.stageSettingCode and fs.ClassCode=? and fs.AccyearCode=? and fs.Termcode=? and fsd.stagecode in (select StageId from campus_student where student_admissionno_var=?)";
	...
	public static final String GET_FEECOLLECTION_TOTAL_AMOUNT = "SELECT SUM(feeAmount) totalFeeAmount FROM campus_fee_setupdetails WHERE feeSettingCode=? AND feeCode NOT IN (SELECT feeCode FROM `campus_fee_specialfee_setup` WHERE `student_id`=? AND `accyear`=? AND termid=?)";
