$(document).ready(function() {
	...
	$("select.teacher").focus(function(){
		getTeacherList($(this),$(this).attr("name").slice(0, -1));
	});
	    			 ...
	    			 success : function(data) {
	    				 ...
	    				 if (result.timetable_Result == "success") {
	    					 ...
	    					 setInterval(function() {
	    						 ...
	    						 window.location.href = "adminMenu.html?method=gettodaytimetablelist";
...
function getTeacherList(pointer,data){
	var timetableId=window.location.href.substring(window.location.href.lastIndexOf("=")+1);
	 ...
	 $.ajax({
			type : 'POST',
			url : "adminMenu.html?method=getTodayTeacherList",
			data:{'tperiod':data,
					'timetableId':timetableId
			},
			async : false,
			...
			success : function(response) {
				...
				pointer.append("<option value=''>-----Select----</option>");
					...
					for(var i=0;i<len;i++){
						pointer.append("<option value='"+result.teacherList[i].teacherId+"'>"+result.teacherList[i].teacherName+"</option>");
					  }	
					}
...
function getTeacherListToday(){
	var timetableId=window.location.href.substring(window.location.href.lastIndexOf("=")+1);
	
	 $.ajax({
			type : 'POST',
			url : "adminMenu.html?method=getTodayTeacherListToday",
