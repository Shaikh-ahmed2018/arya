...
//DOWNLOAD PDF AND EXCEL
	...
	$("#pdfDownload").click(function(){
		...
		window.location.href = "addfee.html?method=doublePaymentDetailsPDF" +
				"&locId="+locId+"&accId="+accId+"&classId="+classId+"&divId="+divId+"&termId="+termId
			   +"&locname="+locname+"&classname="+classname+"&divname="+divname+"&termname="+termname+"&accname="+accname;
	});
	$("#print").click(function(){
		var location=$("#locationName option:selected").text(); 
		var reportType="Double Payment Student List";
		var accyear=$("#academicYear option:selected").text();
		 ...
		 var b= document.getElementById("collapseOne").innerHTML;
	  	   var abd= b;
	  	    var frame1 = $('<iframe />');
		        ...
		        frameDoc.document.write('<link href="CSS/newUI/custome.css" rel="stylesheet">');
		        frameDoc.document.write('<script type="text/javascript" src="JS/newUI/bootstrap.min.js"></script>');
		        frameDoc.document.write('<script type="text/javascript" src="JS/backOffice/Reports/ExamClassWiseReport.js"></script>');
		        frameDoc.document.write('<style>table td,table th{border:1px solid #000 !important;}</style>');
		        frameDoc.document.write('</head><body>');
	...
	$("#studentName").autocomplete({
		...
		select : function(event, ui) {
			...
			getDoublePaymentListbyId(searchTerm);
...
function getDoublePaymentListbyId(studentId){
	datalist={
	...
	$.ajax({		
		type:"post",
		url:"feecollection.html?method=getDoublePaymentListbyId",
		data:datalist,
		async:false,
		success:function(data){
			...
			if(result.data.length>0){
				for(var i=0;i<result.data.length;i++){
					$("#allstudent tbody").append("<tr>"
							...
							+"<td>"+result.data[i].transId+"</td>"
							+"<td>"+result.data[i].bank+"</td>"
							+"<td>"+result.data[i].totalAmt+"</td>"
...
function getDoublePaymentList()
{
	...
	$.ajax({
		...
		type:"post",
		url:"feecollection.html?method=getDoublePaymentList",
		data:datalist,
		async:false,
		success:function(data){
